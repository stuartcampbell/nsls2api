...
async def _call_ups_servicenow_webservice(url: str):
    ...
    return await _call_async_webservice_with_client(url, auth=(username, password), headers=headers, client=httpx_client_wrapper())
...
async def get_proposal_types(
    ...
    if not ups_facility_id:
        error_message: str = f"Facility {facility} does not have a Universal Proposal System ID."
...
async def get_facility_details(facility: UpsFacilityName = UpsFacilityName.nsls2,):
    ups_facility_id = await facility_service.ups_id_for_facility(facility_name=facility)
